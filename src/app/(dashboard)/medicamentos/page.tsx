...
export default function MedicationsPage() {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<MedicationInput>({ resolver: zodResolver(medicationSchema) });
  ...
  const exportPdf = () => {
    ...
    autoTable(doc, {
      startY: 28,
      head: [
        ["NOME", "FABRICANTE", "DOSAGEM", "CATEGORIA", "ESTOQUE", "PRECO"],
      ],
            ...
            >
              <DialogTrigger render={<Button />}>
                <span className={formStyles.button.trigger}>
                  <HiPlus className="h-3.5 w-3.5" /> NOVO MEDICAMENTO
                </span>
              </DialogTrigger>
            ...
            <TableBody>
              {medications.map((m) => (
                <TableRow key={m.id} className={formStyles.table.bodyRow}>
                  ...
                  <TableCell>
                    <span
                      className={`font-mono text-[11px] font-bold ${m.stockQuantity < 10 ? "text-[#E61919]" : "text-[#EAEAEA]"}`}
                    >
                      {m.stockQuantity}
                    </span>
                  </TableCell>
