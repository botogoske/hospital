...
const coverageBorders: Record<string, string> = {
  BASIC: "border-l-[#555555]",
  STANDARD: "border-l-[#E61919]",
  PREMIUM: "border-l-[#E61919]",
};
...
export default function HealthPlansPage() {
  ...
  const handleEdit = (plan: HealthPlan) => {
    ...
    setValue(
      "coverageType",
      plan.coverageType as "BASIC" | "STANDARD" | "PREMIUM",
    );
  ...
  const filteredPlans = healthPlans.filter(
    (p) =>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={formStyles.header.subtitle}>
                GERENCIE OS CONVENIOS E PLANOS DE SAUDE
              </p>
            </div>
          </div>
            ...
            >
              <DialogTrigger render={<Button />}>
                <span className={formStyles.button.trigger}>
                  <HiPlus className="h-3.5 w-3.5" /> NOVO PLANO
                </span>
              </DialogTrigger>
              <DialogContent className={formStyles.dialog.content}>
                <DialogHeader className={formStyles.dialog.header}>
                  <DialogTitle className={formStyles.dialog.title}>
                    {editingPlan
                      ? "[ EDITAR ] PLANO DE SAUDE"
                      : "[ NOVO ] CADASTRAR PLANO DE SAUDE"}
                  </DialogTitle>
                </DialogHeader>
                <form
                  ...
                  {editingPlan && (
                    <div className="flex items-center gap-2 border border-[#222222] bg-[#0D0D0D] px-3 py-2">
                      <input
                        ...
                        className="h-3.5 w-3.5 accent-[#E61919]"
                      />
                      <Label
                        htmlFor="isActive"
                        className="font-mono text-[10px] uppercase tracking-[0.1em] text-[#EAEAEA]"
                      >
                        PLANO ATIVO
                      </Label>
                    </div>
            ...
            <TableBody>
              {filteredPlans.map((plan) => (
                <TableRow key={plan.id} className={formStyles.table.bodyRow}>
                  ...
                  <TableCell>
                    <span
                      className={`${statusBadgeStyle} ${statusColors[plan.coverageType.toLowerCase() as keyof typeof statusColors] || "border-l-[#333333]"}`}
                    >
                      [ {coverageLabels[plan.coverageType]} ]
                    </span>
                  </TableCell>
                  <TableCell className={formStyles.table.cell}>
                    {plan._count.appointments}
                  </TableCell>
                  <TableCell>
                    <span
                      className={`font-mono text-[10px] uppercase tracking-wider ${plan.isActive ? "text-[#4AF626]" : "text-[#E61919]"}`}
                    >
