...
const bedStatusBorders: Record<string, string> = {
...
export default function BedsPage() {
  ...
  const handleEdit = (bed: Bed) => {
    setEditingBed(bed);
    setValue("number", bed.number);
    setValue("ward", bed.ward);
    setValue("floor", bed.floor);
    setValue(
      "bedType",
      bed.bedType as "REGULAR" | "ICU" | "EMERGENCY" | "PEDIATRIC",
    );
    setSelectedType(bed.bedType);
    setOpen(true);
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={formStyles.header.subtitle}>
                GERENCIAMENTO DE LEITOS DO HOSPITAL
              </p>
            </div>
          </div>
            ...
            >
              <DialogTrigger render={<Button />}>
                <span className={formStyles.button.trigger}>
                  <HiPlus className="h-3.5 w-3.5" /> NOVO LEITO
                </span>
              </DialogTrigger>
              <DialogContent className={formStyles.dialog.content}>
                ...
                >
                  ...
                  <div className={formStyles.field.wrapper}>
                    <Label className={formStyles.field.label}>&gt; ALA</Label>
                    <Input
                      {...register("ward")}
                  ...
                  <div className={formStyles.field.wrapper}>
                    <Label className={formStyles.field.label}>&gt; ANDAR</Label>
                    <Input
                      type="number"
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setValue(
                          "bedType",
                          e.target.value as
                            "REGULAR" | "ICU" | "EMERGENCY" | "PEDIATRIC",
                        );
                      }}
      ...
      <div className={formStyles.stats.container}>
        {[
          ...
          <div key={s.label} className={formStyles.stats.item}>
            <div className={`${formStyles.stats.value} ${s.color}`}>
              {s.value}
            </div>
      ...
      <div className={formStyles.section.container}>
        <div className={formStyles.section.header}>
          ...
          <div className={formStyles.search.wrapper}>
            ...
            <input
              placeholder="BUSCAR POR NUMERO OU ALA..."
              value={search}
            ...
            <TableBody>
              {filteredBeds.map((bed) => (
                <TableRow key={bed.id} className={formStyles.table.bodyRow}>
                  <TableCell className={formStyles.table.cellBold}>
                    {bed.number}
                  </TableCell>
                  ...
                  <TableCell className={formStyles.table.cellMuted}>
                    {bedTypeLabels[bed.bedType]}
                  </TableCell>
                  <TableCell>
                    <span
                      className={`${statusBadgeStyle} ${statusColors[bed.status.toLowerCase() as keyof typeof statusColors] || "border-l-[#333333]"}`}
                    >
